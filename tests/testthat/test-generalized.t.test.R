...
for(i in 1:10000) {
    ...
    gtt_res_0_eq <- c(gtt_res_0_eq,
                      generalized.ttest(z[conds==1], z[conds==0], 0, 0, equal.var=T)$p) 
    gtt_res_i <- c(gtt_res_i,
                   generalized.ttest(z[conds==1], z[conds==0], epsilon_l, epsilon_u,
                                      equal.var=T)$p)
    gtt_meh_res_i <- c(gtt_meh_res_i,
                       generalized.ttest.mehring(z, conds, epsilon_l, epsilon_u)$p)
}
...
for(i in 1:10000) {
    ...
    gtt_res_0_neq <- c(gtt_res_0_neq,
                       generalized.ttest(y, x, 0, 0, equal.var=F)$p)
}
